...
@NamedQueries({
    ...
    @NamedQuery(name = "CurrencyMast.dropdownlist", query = "SELECT c FROM CurrencyMast c WHERE c.currNameAr = :currNameAr AND c.currName = :currName AND c.currCode = :currCode AND  c.currStatus = :currStatus" ),
...
public class CurrencyMast implements Serializable {
    ...
    @OneToMany(mappedBy = "srvCurrCode" ,targetEntity=ServiceMast.class)
