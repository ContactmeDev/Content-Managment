...
public class LanguageMastFacade extends AbstractFacade<LanguageMast> {
    ...
    public boolean recordExist(String lanCode){
        List<LanguageMast> result = em.createNamedQuery("LanguageMast.findByLanCode").setParameter("lanCode", lanCode).getResultList();
        if (result.size() > 0)
            return true;
        else
            return false;
    } 
    public List<LanguageMast> getDDList(){
        return em.createNamedQuery("LanguageMast.findByLanStatus").setParameter("lanStatus", "A").getResultList();
    }
